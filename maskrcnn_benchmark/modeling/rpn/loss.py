...
class RPNTripletLossComputation(object):
    """
    This class computes the RPN Triplet loss.
    """
    TRIPLET_MARGIN = 1
    ...
    def __call__(self, anchors, objectness, box_regression, targets, embeddings):
        """
        Arguments:
            anchors (list[BoxList])
...
#         print("===============================================",objectness[sampled_inds].shape, labels[sampled_inds].shape)
...
#         margin = 25.0
        margin = RPNTripletLossComputation.TRIPLET_MARGIN
...
#             RPNTripletLossComputation.TRIPLET_MARGIN += 1 
        ...
        return objectness_loss, box_loss, triplet_loss

class RPNContrastiveLossComputation(object):
    """
    This class computes the RPN Siamese loss.
    """
    PAIR_MARGIN = 1
    def __init__(self, proposal_matcher, fg_bg_sampler, box_coder,
                 generate_labels_func):
...
def make_rpn_loss_evaluator(cfg, box_coder, mode = 0):
    ...
    if mode == 0:
        ...
        loss_evaluator = RPNContrastiveLossComputation(
            matcher,
            fg_bg_sampler,
            box_coder,
            generate_rpn_labels
        ...
        loss_evaluator = RPNTripletLossComputation(
            matcher,
            fg_bg_sampler,
            box_coder,
            generate_rpn_labels
        ...
        raise RuntimeError("loss mode should be interger in [0, 2], not available: {}".format(mode))
