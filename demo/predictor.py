...
class HistoDemo(object):
    # Histo categories for pretty print
    CATEGORIES = [
        "__background",
        "rc",
    ]
    ...
    def __init__(
        ...
        save_dir = os.path.join(cfg.OUTPUT_DIR, cfg.OUTPUT_FOLDER)
        checkpointer = DetectronCheckpointer(cfg, self.model, save_dir=save_dir)
        _ = checkpointer.load(cfg.MODEL.WEIGHT, use_latest=False)
    ...
    def run_on_pil_image(self, image):
        ...
        Returns:
            prediction (BoxList): the detected objects. Additional information
                ...
                the BoxList via `prediction.fields()`
        """
        predictions = self.sliding_window_wsi(image)
        top_predictions = self.select_top_predictions(predictions)
        
        # transfer to cv2 image
    ...
    def sliding_window_wsi(self, pil_image, nms_thresh = 0.3, model_size = 600, overlap = 200):
...
#                     l_labels.append(boxlist.get_field('objectness')>0.5)
    ...
    def compute_prediction(self, original_image):
        """
        Arguments:
            prediction (BoxList): the detecte
            original_image (np.ndarray): an image as returned by OpenCV

        Returns:d objects. Additional information
                of the detection properties can be found in the fields of
                the BoxList via `prediction.fields()`
        """
        # apply pre-processing to image
